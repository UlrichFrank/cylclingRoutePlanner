...
export const RouteMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (mapInstance.current && currentRoute) {
      ...
      if (currentRoute.coordinates.length > 1) {
        const latlngs = currentRoute.coordinates.map((coord) => [coord.lat, coord.lng] as [number, number]);
        const polyline = L.polyline(latlngs, { color: 'blue', weight: 3, opacity: 0.7 });
        ...
        currentRoute.coordinates.forEach((coord, idx) => {
    ...
    // Handle clicks on map container directly (failsafe if Leaflet doesn't fire)
    const mapEl = mapContainer.current;
    const handleMapClick = (e: MouseEvent) => {
      console.log('[MapContainer] Direct click event fired!', e.button);
      if (e.button === 0) { // left click
        console.log('[MapContainer] Left click detected, calculating coordinates');
        ...
        if (mapInstance.current && mapEl) {
          ...
          console.log('[MapContainer] Calculated latlng:', latlng);
    ...
    if (mapEl) {
      mapEl.addEventListener('click', handleMapClick, true); // use capture phase
    }
